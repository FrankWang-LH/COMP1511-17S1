...
void get_action(struct bot *bot, int *action, int *n){
    ...
    int fuel = nearest_fuel(&bot[0]);// distance to nearest fuel
    ...
    if((gas(&bot[0]) == 0 && bot -> fuel == 0) || (gas(&bot[0])  <= CRITICAL_FIGURE && bot -> fuel <= CRITICAL_FIGURE && bot -> cargo == NULL) || (bot -> turns_left <= 2 && bot->cargo == NULL)){
        *action = ACTION_MOVE;
        *n = 0;

    // refuel    
    } else if(bot -> fuel <= bot -> fuel_tank_capacity*9/50 && bot -> location -> type != LOCATION_PETROL_STATION && gas(&bot[0]) > 0){
    ...
    // find seller when the bot is empty
        ...
        l = bot -> location;
        seller = nearest_seller(&bot[0]);// calculate distance to nearest seller
        *action = ACTION_MOVE;
        *n = seller;
    // move to buyer when the bot is carrying commodity and haven't reach the buyer
    } else if(l -> type != LOCATION_BUYER && l -> type != LOCATION_SELLER && bot -> cargo != NULL){
        l = bot -> location;
        buyer = nearest_buyer(&bot[0]);// calculate distance to nearest buyer
        *action = ACTION_MOVE;
        *n = buyer;
    // buy things after reaching seller
    } else if(l -> type == LOCATION_SELLER && bot -> cargo == NULL && l -> quantity != 0 && bot -> fuel >= nearest_buyer(&bot[0])){
        l = bot -> location;
        needs = 0;
        *action = ACTION_BUY;
        needs = need(&bot[0]);// calculate need for the commodity at this location
        *n = needs;
